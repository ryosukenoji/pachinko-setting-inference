...
このツールは性質の違う2つの +EV 手法を別エンジンで実装する。**機種タイプで使う軸が決まる**。

| | 軸A: 設定看破 | 軸B: 天井・ゾーン狙い |
|---|---|---|
| 問題 | 隠れた**設定**を推定（ベイズ） | 公表**天井**との差を引き算（決定論） |
| 隠れ変数 | あり（設定を推定する） | **なし**（天井は公表・現在Gは表示） |
| 向く機種 | ノーマルAタイプ（**ジャグラー系**） | 天井のある機種（**GOD系等のAT機**） |
| 観測 | N回転・BIG/REG・小役 | 現在ゲーム数 |
| 出力 | 設定事後 → 円建てEV + 収支帯 | 残りEV + 狙い目ボーダーG |
| コマンド | `infer` / `daily` | `ceiling` |
| 状態 | ✅ 3機種照合済 | ✅ パラメータ駆動 |

- **軸A**: ジャグラー系で「設定を想定 → いくら投資でいくらバックか」を期待値＋分布で出す（`infer --play-games` / `--recover-yen` / `daily`）。
- **軸B**: GOD系など天井機で「この捨て台、何G以上回ってれば拾って+EVか」を出す（`ceiling`）。設定推定は不要（天井は設定にほぼ依存しない固定値）。
- なぜ分けるか: AT機（GOD等）は有利区間・モード・天井で各ゲームが独立でなく、軸Aのベイズ前提が崩れる。だから軸Bの決定論で解く。詳細は [`docs/design_proposal.md`](docs/design_proposal.md) §3。
...
# 軸B: 天井・ゾーン狙い（GOD系等）。現在G → 残りEV / 狙い目ボーダー
python3 -m src.cli ceiling --ceiling 1200 --current 900 --benefit-coins 2000
#   → 残り300G / 期待投資9,091円 / 恩恵40,000円 / 期待収支+30,909円 → 打つ
# 通常時初当りを織り込む（早い当たりで投資が減る）
python3 -m src.cli ceiling --ceiling 1200 --current 700 --benefit-coins 2000 \
    --hit-one-in 400 --early-coins 800
#   ※ 設定推定は不要（天井は設定にほぼ依存しない固定値）。値は機種スペックを入れる

# 登録機種の一覧 + テーブル整合性チェック（軸A）
