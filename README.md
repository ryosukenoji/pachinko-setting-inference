...
|---|---|---|
| M0 | 構造案・本README | done |
| M1 | **尤度テーブル整備** — マイジャグラーVの「観測量×設定」理論確率を [`data/machines/my_juggler_v.json`](data/machines/my_juggler_v.json) に（2ソース照合済み `cross-checked`。ぶどうは公式非公表の実測推定。アイムEXは中間設定 `[unverified]`） | done |
...
- 尤度テーブルの裏取り状況: **マイジャグラーV = 2ソース照合済み**（ぶどうのみ公式非公表の実測推定）。**アイムジャグラーEX = 端点のみ照合・中間設定とぶどうは `[unverified]`**（次の作業候補）。
