...
| Milestone | 内容 | 状態 |
|---|---|---|
| M0 | 構造案・本README（本コミット） | done |
| M1 | **尤度テーブル整備** — 1機種（ジャグラー系推奨：ぶどう確率の設定差が明確）の「観測量×設定」理論確率を `data/machines/` に | todo |
| M2 | **ベイズ事後エンジン** — multinomial/binomial 尤度 × 事前 → 設定事後分布（`posterior.py`） | todo |
| M3 | **EV判定** — 事後 × 公表機械割 → 打つ/やめる。パチンコは回転率→ボーダー判定（`ev.py`） | todo |
| M4 | **検証** — ネガコン・キャリブレーション・必要サンプル・EVバックテスト（`validate.py`、Loto6の `evaluation.py` 移植） | todo |
| M5 | **データ取得** — 手入力 → CSV取込 → （可能なら）自動収集。UI/CLI | todo |
| M6 | 複数機種対応・ホール事前分布の学習 | todo |

最小で価値が出るのは **M1〜M4**（1機種で「推定が正しいことを検証済み」の状態）。
