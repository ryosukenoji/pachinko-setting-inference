...
|---|---|---|
| M0 | 構造案・本README | done |
| M1 | **尤度テーブル整備** — [マイジャグラーV](data/machines/my_juggler_v.json)・[アイムジャグラーEX](data/machines/im_juggler_ex.json) の「観測量×設定」理論確率（いずれも2ソース照合済み `cross-checked`。ぶどうは公式非公表の実測推定） | done |
...
# 円建て期待収支も出す（あと5000G回す想定 / 3枚掛け / 等価20円）
python3 -m src.cli infer --machine data/machines/my_juggler_v.json \
    --games 4000 --BIG 17 --REG 16 --budou 690 \
    --play-games 5000 --bet 3 --coin-yen 20
#   → 期待収支 +12,034円 / プラス確率75% / 幅 -9,000円(設定1)〜+28,200円(設定6)
#   ※ 設定不確実性のみ反映。短期分散(ヒキ)は含まない
...
- 尤度テーブルの裏取り状況: **マイジャグラーV / アイムジャグラーEX とも2ソース照合済み**（`cross-checked`）。ぶどうは両機種とも公式非公表の実測推定（サイト間でばらつく）。アイムEXは「ぶどうが設定1-5でほぼ無差・設定6のみ優遇」で、ぶどうは設定6フラグとしてのみ機能。
